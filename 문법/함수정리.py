...
# 반복문 = 기준에 따른 반복
    ...
    # 2. for 변수명 in range(시작값 , 끝값 , 증감단위 ) :  시작값 부터 끝값까지 증감하면서 변수명에 하나씩 대입
                    # range( 값 ) : 값 만큼 반복 실행
                    # range ( 시작값 , 끝값 ) : 시작값 부터 끝값전 까지 1씩 증가하면서 반복
                    # range ( 시작값 , 끝값 , 증감단위 ) : 시작값 부터 끝값까지 증감하면서 변수명에 하나씩 대입
    # 2 ~ 9단 구구단
for 단 in range(2,10) :  # 2 부터 9 까지 1씩 증가 하면서 단에 하나씩 대입
    for 곱 in range(1,10) :  # 1 부터 9까지 1씩 증가 하면서 곱에 하나씩 대입
        print(단,"x",곱,"=",단*곱)

    # 피라미드 만들기
for 줄 in range(6) : # 5줄 만들기
    print( " "*(6-줄) , "*"*(2*줄-1))
...
# 배수 구하는 법
# 배수는 내가 원하는 숫자 % 알고 싶은 숫자가 0이라면  배수이다

# 홀수 짝수 구별법
# 알고싶은수 % 2 가 0이면 짝수 0이 아니면 홀수이다